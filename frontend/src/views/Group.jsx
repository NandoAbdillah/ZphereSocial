...
import CreateGroupPost from "../assets/CreateGroupPost";
...
export default function Group() {
  ...
  useEffect(() => {
    axiosClient
      .get("/group-profile", { params: { uuid: uuid } })
      .then((response) => {
  ...
  const handleLike = async (postId) => {
    try {
      ...
      const response = await axiosClient.get("/group-profile", {
        params: { uuid: uuid },
      });
      ...
      setGroup({ ...response.data.data, posts: updatedPosts });
    } catch (error) {
  ...
  const handleSavePost = (postId) => {
    ...
    const updatedPosts = group.posts.map((post) =>
  ...
  const handleCommentSubmit = (postId, e) => {
    ...
    setGroup({ ...group, posts: updatedPosts });
  ...
  useEffect(() => {
    console.log("User post updated:", group);
  }, [group]);
            ...
            <div className="card w-100 shadow-xss rounded-xxl overflow-hidden border-0 mb-3 mt-3 pb-3">
              ...
              <div className="card-body d-block pt-4 text-center">
                <figure className="avatar mt--6 position-relative w75 z-index-1 w100 z-index-1 ms-auto me-auto">
                  <img
                    src={`${import.meta.env.VITE_ASSET_URL}${group.icon}`}
                    alt="image"
              ...
              <div className="card-body d-flex align-items-center ps-4 pe-4 pt-0 mx-auto">
                ...
                <h4 className="font-xsssss text-center text-grey-500 fw-600 ms-2 me-2">
                  <a href={`/group-members/${group.uuid}`}>
                    <b className="text-grey-900 mb-1 font-xss fw-700 d-inline-block ls-3 text-dark">
                      {group.members ?? "0"}
                    </b>{" "}
                    Members
              ...
              <div className="card-body d-flex align-items-center justify-content-center ps-4 pe-4 pt-0">
                ...
                {groupStatus[group.id] &&
                  (groupStatus[group.id].status === "hasJoin" ? (
                    <button
                      onClick={() => leaveGroup(group.id)}
                      className="btn text-center p-2 lh-24 w100 ms-1 ls-3 d-inline-block rounded-3 bg-warning font-xsssss fw-700 ls-lg text-white"
                    >
                      LEAVE
                    </button>
                  ) : (
                    <button
                      ...
                      className="btn text-center p-2 lh-24 w100 ms-1 ls-3 d-inline-block rounded-3 bg-info font-xsssss fw-700 ls-lg text-white"
                    >
                      JOIN
                    </button>
                  ))}
                {/* <a
                                ...
                                >
                                    <i className="feather-bell font-md" />
                                </a>
                                <a
                                    href="#"
