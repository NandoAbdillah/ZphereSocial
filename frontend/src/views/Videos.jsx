...
export default function Videos() {
  ...
  const loadPost = async (page = 1) => {
    ...
    try {
      const response = await axiosClient.get(`/videos?page=${page}`);
  ...
  const videoJsOptions = {
    ...
    fluid: true,
    preload: 'metadata',
    liveui: false,  // Pastikan live UI dimatikan
    sources: [
      {
        src: "images/v-2.mp4",
        type: "video/mp4",
  ...
  const handlePlayerReady = (player) => {
    ...
    // Cek jika metadata sudah ter-load
    player.on("loadedmetadata", () => {
      videojs.log("Video metadata loaded, ready to seek.");
      videojs.log(`Video duration: ${player.duration()} seconds`);
    });

    // Cek apakah video sedang buffering
    player.on("progress", () => {
      const buffered = player.buffered();
      if (buffered.length > 0) {
        videojs.log(`Buffered up to: ${buffered.end(0)} seconds`);
      }
    });

    // Event listener ketika video buffering
    player.on("waiting", () => {
      videojs.log("player is waiting");
    });

    // Event listener saat player di-dispose
    player.on("dispose", () => {
      videojs.log("player will dispose");
    });

    // Event listener untuk memantau waktu saat video diputar
    player.on("timeupdate", () => {
      videojs.log(`Current time: ${player.currentTime()} seconds`);
    });
  };
    ...
    axiosClient
      ...
      .then((response) => {
        ...
        setPosts(
          posts.map((post) =>
            post.id === postId
              ? { ...post, content: contentText } // perbaikan di sini
              : post
          )
        );
        $(`#Edit${postId}`).modal("hide");
  ...
  const deletePost = (postId) => {
    axiosClient
      ...
      .then((response) => {
        setPosts(
          posts.filter((post) => post.id !== postId) // Hanya menyisakan produk yang id-nya tidak sama dengan yang dihapus
        );
        openNotificationWithIcon("success", response.data.message);
      })
      .catch((err) => {
